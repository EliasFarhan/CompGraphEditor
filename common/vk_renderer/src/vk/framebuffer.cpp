...
void Framebuffer::Load(const core::pb::FrameBuffer& framebufferPb)
{
    ...
    for(auto& targetInfo: framebufferPb.color_attachments())
    {
        ...
        renderTarget.image = engine.CreateImage(targetSize.x,
            ...
            VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | (targetInfo.rbo() ? 0:VK_IMAGE_USAGE_SAMPLED_BIT),
    ...
    if(framebufferPb.has_depth_stencil_attachment())
    {
        ...
        renderTarget.image = engine.CreateImage(targetSize.x,
            ...
            1,
            VK_IMAGE_TILING_OPTIMAL,
            VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | (targetInfo.rbo() ? 0:VK_IMAGE_USAGE_SAMPLED_BIT),
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
            1);
        renderTarget.imageView = CreateImageView(driver.device,
            renderTarget.image.image,
