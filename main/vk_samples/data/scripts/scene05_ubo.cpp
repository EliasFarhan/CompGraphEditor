...
extern "C"
{
    ...
    void WASM_EXPORT scene05_begin()
    {
        t = 0.0f;
    }

    void WASM_EXPORT scene05_update(float dt)
    {
        t += dt;
    }

    void WASM_EXPORT scene05_draw(int64_t drawCommandId)
    {
        script::DrawCommand drawCommand(drawCommandId);
        drawCommand.Bind();
        auto view = glm::mat4(1.0f);
        view = glm::translate(view, glm::vec3(0,0,-5));
        drawCommand.SetMat4("ubo.view", view);
        ...
        for (auto& position : positions)
        {
            ...
            drawCommand.SetMat4("constant_values.model", model);
            drawCommand.Draw();
        }
    }
}